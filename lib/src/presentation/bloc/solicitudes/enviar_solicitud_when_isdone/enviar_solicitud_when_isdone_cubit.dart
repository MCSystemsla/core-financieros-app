    ...
    try {
      ...
      for (var solicitud in solicitudes) {
        try {
          ...
          //   continue;
          // }
          if (solicitud is ResponseLocalDb) {
            result = await repository.createSolicitudCreditoNuevaMenor(
              solicitudNuevaMenor: SolicitudNuevaMenor(
                ...
                telefonoTrabajoConyugue:
                    solicitud.telefonoTrabajoConyugue ?? '',
                ...
                ubicacionGradosLongitud:
                    solicitud.ubicacionGradosLongitud ?? '',
                ...
                parentescoFamiliarPeps2:
                    solicitud.parentescoFamiliarPeps2 ?? '',
          ...
          } else if (solicitud is ReprestamoResponsesLocalDb) {
            result = await repository.createSolicitudReprestamo(
              solicitudReprestamo: SolicitudReprestamo(
                ...
                fechaPrimerPagoSolicitud: DateTime.tryParse(
                    solicitud.fechaPrimerPagoSolicitud.toString()),
              ),
            );
          }
          if (!result.$1) {
            objectBoxService.updateWhenSolicitdIsFailed(
                solicitudId: solicitud.id, errorMsg: result.$2);
            emit(OnEnviarSolicitudWhenIsdoneError(
                msgError:
                    // 'Error en Solicitud de ${solicitud.nombre1} ${solicitud.apellido1}: ${result.$2}'));
                    'Error en Solicitud: ${result.$2}'));
            return;
          }
