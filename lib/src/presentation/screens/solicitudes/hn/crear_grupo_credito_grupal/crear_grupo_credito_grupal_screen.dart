  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text('Crear Grupo de Crédito'),
        ),
        body: BlocBuilder<GruposActivosCubit, GruposActivosState>(
          builder: (context, state) {
            return switch (state.status) {
              ...
              Status.done => ListView.builder(
                  padding: const EdgeInsets.all(15),
                  itemCount: state.gruposActivos.length,
                  itemBuilder: (BuildContext context, int index) {
                    return SelectableCardItem(
                      color: const Color(0xFF1565C0),
                      icon: Icons.group,
                      title: state.gruposActivos[index].nombreCompleto,
                      subtitle: 'Codigo: ${state.gruposActivos[index].codigo}',
                      onTap: () {},
                    );
                  },
                ),
