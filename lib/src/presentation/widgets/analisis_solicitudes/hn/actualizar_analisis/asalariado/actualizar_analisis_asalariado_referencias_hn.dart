...
class ActualizarAnalisisAsalariadoReferenciasHn extends StatefulWidget {
  final PageController pageController;
  const ActualizarAnalisisAsalariadoReferenciasHn({
    super.key,
    required this.pageController,
  });

  @override
  State<ActualizarAnalisisAsalariadoReferenciasHn> createState() =>
      _ActualizarAnalisisAsalariadoReferenciasHnState();
}

class _ActualizarAnalisisAsalariadoReferenciasHnState
    extends State<ActualizarAnalisisAsalariadoReferenciasHn> {
  late final _initialState =
      context.read<ActualizarAnalisisAsalariadoHnCubit>().state;
  ...
  Future<void> selectFechaVerificacion(BuildContext context) async {
    final cubit = context.read<ActualizarAnalisisAsalariadoHnCubit>();
  ...
  Future<void> selectFechaVerificacion3(BuildContext context) async {
    ...
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate: fechaVerificacion3 ?? DateTime.now(),
      firstDate: DateTime(2000),
      lastDate: DateTime.now(),
      locale: Locale(context.read<LangCubit>().state.currentLang.languageCode),
    );
  ...
  Widget build(BuildContext context) {
    final cubit = context.read<ActualizarAnalisisAsalariadoHnCubit>();
    return BlocConsumer<ActualizarAnalisisAsalariadoHnCubit,
                      ...
                      children: [
                        CustomElevatedButton(
                          onPressed: () {
                            ...
                            cubit.actualizarAnalisisAsalariado();
                          },
